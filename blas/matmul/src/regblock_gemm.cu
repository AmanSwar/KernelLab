...
__global__ void blocked_gemm(float *A , float *B , float *C , int M , int N , int K , int alpha , int  beta){
    ...
    //diving into number of tiles
    for(int tile = 0 ; tile < (K + BLOCK_SIZE -1) / BLOCK_SIZE ; tile++){

        // bring elements to shared mem
        ...
        // unloop for registers (block size => 64 = #regs)
        #pragma unroll
