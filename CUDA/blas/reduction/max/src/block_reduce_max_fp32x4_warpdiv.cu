...
__device__ float block_max_fp32x4(float *arr , int N){
    ...
    value.x = (global_index + 0 < N) ? arr[global_index + 0] : -FLT_MAX;
    value.y = (global_index + 1 < N) ? arr[global_index + 1] : -FLT_MAX;
    value.z = (global_index + 2 < N) ? arr[global_index + 2] : -FLT_MAX;
    value.w = (global_index + 3 < N) ? arr[global_index + 3] : -FLT_MAX;
