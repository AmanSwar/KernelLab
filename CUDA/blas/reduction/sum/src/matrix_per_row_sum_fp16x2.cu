...
__global__ void matrix_reduce_sum_fp16x8_kernel(
    ...
    int local_index_x = threadIdx.x * 2; // range -> 0 - blockDim.x
    ...
    for(int tile = 0 ; tile < total_tiles ; tile ++){
        ...
        for(int i = 0 ; i < 4 ; i++){
            half2 element;
            if (col_idx < N){  
                element = HALF2(input_matrix[row_start + col_idx + i]);
...
void launch_matrix_reduce_sum_fp16x8(
    half* input_matrix,
    half* output_arr,
    int M , int N
){
